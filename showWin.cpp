...
void renumberPieces(PieceIdType *board, PosType *curLoc) {
   ...
   if (firstTime) {
       prevLoc = (PosType *)malloc(sizeof(PosType) * numPiece);
       test(prevLoc != NULL, "can't malloc prevLoc in renumberPieces()");
...
void showWin(BufferId inBId, uint64_t offset) {
	...
	PieceIdType movedPiece;
	PosType movedUp, movedRight;
	wt.renumberAndFindDiff(&movedPiece, &movedUp, &movedRight);
	wt.printBoard();
    }
}
